...
## Project Overview
This repository documents the official rules, entities, and mechanics for multiple games (e.g., Chess, Pokémon TCG, Chessemon) in a modular, Markdown-based format. Each game is organized in its own top-level folder (e.g., `chess/`, `pokemon-tcg/`, `chessemon/`), with all documentation self-contained and structured for clarity and reference. No executable code is present; this is a documentation-first codebase.

## Architecture & Structure
- **Per-game folder structure:**
  - Each game (e.g., `chess/`, `pokemon-tcg/`, `chessemon/`) contains:
    ...
    - `research/`: Reserved for experimental or future documentation.
    - `dev/` (chessemon only): Development plans and implementation guidance for code integration (no source code).
  - Each subfolder is self-contained; cross-references are only within the same folder unless explicitly listed in the folder's `README.md`.
  - Subdirectories are not referenced unless listed in the folder's `README.md`.
...
- For development plans (chessemon), see `dev/implementation-plan.md` and `dev/README.md` for technical guidance; do not add code or config files to `dev/`.
...
- For technical implementation planning (chessemon), update `dev/implementation-plan.md` and reference only files within `dev/`.

## Integration & External Dependencies
- No external dependencies or integrations are required for any documentation in this repository.
- For code integration planning (chessemon), reference boardgame.io and React only in documentation; do not add code or config files.
...
- **Update development plans (chessemon):** Edit `dev/implementation-plan.md` and `dev/README.md`.
